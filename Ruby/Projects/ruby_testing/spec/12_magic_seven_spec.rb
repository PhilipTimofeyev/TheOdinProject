...
describe MagicSeven do
  ...
  describe '#subtract_random_number' do
    it 'returns 3' do
      new_game = described_class.new(5)
      number = 8

      result = new_game.subtract_random_number(number)
  ...
  describe '#play' do
    ...
    context 'when the random number is 20...' do
      # remove the 'x' before running this test
      subject(:twenty) {described_class.new(20)}
    ...
    context 'when the random number is 1000...' do
      # remove the 'x' before running this test
      subject(:one_thousand) {described_class.new(1000)}
