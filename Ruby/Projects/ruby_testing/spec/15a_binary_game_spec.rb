...
describe BinaryGame do
  ...
  describe '#player_input' do
    ...
    context 'when user inputs an incorrect value once, then a valid input' do
      before do
        allow(game_input).to receive(:gets).and_return('dog', '4')
      end
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        allow(game_input).to receive(:gets).and_return('dog', '?', '7')
      end
  ...
  describe '#verify_input' do
    ...
    subject(:game_verify) {described_class.new(0, 9)}
    ...
    context 'when given a valid input as argument' do
      it 'returns valid input' do
        ...
        expect(game_verify.verify_input(min, max, input)).to be(5)
      end
    end
    ...
    context 'when given invalid input as argument' do
      it 'returns nil' do
        ...
        unverified_input = game_verify.verify_input(min, max, input)
  ...
  describe '#display_turn_order' do
    ...
    subject(:game_display_turn) {described_class.new(0, 10)}
    let(:binary_display) {instance_double(BinarySearch)}

    before do
      allow(binary_display).to receive(:make_guess)
      allow(binary_display).to receive(:update_range)
      allow(game_display_turn).to receive(:display_guess)
      allow(binary_display).to receive(:game_over?)
    end

    # Command Method -> Test the change in the observable state
    it 'increases guess_count by one' do
      expect { game_display_turn.display_turn_order(binary_display) }.to change { game_display_turn.instance_variable_get(:@guess_count)}.by(1)
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends make_guess' do
      expect(binary_display).to receive(:make_guess).once
      game_display_turn.display_turn_order(binary_display)
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends update_range' do
      expect(binary_display).to receive(:update_range).once
      game_display_turn.display_turn_order(binary_display)
    end
